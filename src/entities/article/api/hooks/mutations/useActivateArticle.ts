...
export const useActivateArticle = (id: string): UseMutationResult<unknown, unknown, void, unknown> => {
    return useMutation([MutationKeys.ACTIVATE_ARTICLE], () => articleApi.activateArticle(id), {
        onMutate: async () => {
            ...
            queryClient.setQueriesData<GetAdminArticlesResponse>([QueryKeys.GET_ADMIN_ARTICLES], (previousData) => {
                ...
                return {
                    ...previousData,
                    data: previousData.data.map((article) => (String(article.id) === id ? { ...article, isActive: true } : article)),
                };
            });
